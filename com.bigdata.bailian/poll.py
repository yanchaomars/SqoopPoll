...
ORACLE_TO_HIVE = "sqoop import --hive-import --connect jdbc:oracle:thin:@{ip}:1521:{sid} --username {username}" \
...
def exec_oracle_cmd():
    for line in open("../docs/oracle.txt"):
        ...
        cmd = ORACLE_TO_HIVE.format(ip=placed_line[0], sid=placed_line[2], username=placed_line[3], pwd=placed_line[4], table=placed_line[7], dir=placed_line[6], hive_table=placed_line[6], split_by=placed_line[8], pvalue=get_day("%Y-%m-%d", -1))
        print(cmd)
        Logger.info("Starting import table: " + placed_line[6])
        (status, text) = commands.getstatusoutput(cmd)
        if status != 0:
            Logger.info("End of import, status: " + status)
